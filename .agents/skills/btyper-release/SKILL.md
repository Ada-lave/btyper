...
5. Finish release notes with changes, compatibility, checksums, verification evidence, and untested paths. Close issues and milestone only after their acceptance criteria are satisfied. Central Homebrew/AUR/Scoop registry submissions are separate actions and are not implied by repository recipes.

Before any push, tag, release, issue/milestone mutation, or registry submission, confirm that the action is within the user's requested release scope and that the remote/repository target is correct. Report the exact commit, tag, release URL, recipe commit, checks, and remaining gaps when finished.
